...
local nameplateList = {};
for i,icon in ipairs(FILEMAN:GetDirListing(THEME:GetPathG("","NamePlates").."/")) do
	nameplateList[#nameplateList+1] = {
		setting=icon, display_name=icon:sub(1, -5)
	}
end;

if #nameplateList > 0 then
	menu_items[#menu_items+1] = {name= "Nameplate", get="ProfileFrame", set="ProfileFrame", item_type="customList", list=nameplateList}
else
	SCREENMAN:SystemMessage("WARNING: The profile icon directory appears to be empty.");
end;
